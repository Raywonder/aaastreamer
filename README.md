...
Users can also manage account security from their dashboard: notification email reminders, self-service recovery, built-in authenticator-app 2FA, and passkeys. Admins can edit existing accounts, roles, active state, linked WHMCS details, notification emails, and password resets from the Accounts admin section.
...
- WHMCS invoices: when `AAASTREAMER_WHMCS_*` environment variables are configured, the visitor support form can create a WHMCS invoice for the stream owner's linked WHMCS client ID. Admin-owned streams fall back to the configured platform default client ID so Devine Creations invoices and license-account continuity remain connected.
